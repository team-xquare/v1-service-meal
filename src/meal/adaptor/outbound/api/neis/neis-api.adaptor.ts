...
import { GetMealByDatePort } from "@src/meal/application/port/outbound/get-meal-by-date.port";
import { Meal } from "@src/meal/domain/meal";
import { ExistsByDatePort } from "@src/meal/application/port/outbound/exists-by-date.port";

@Injectable()
export class NeisApiAdaptor implements GetMealByDatePort, ExistsByDatePort {
    constructor(
    ...
    async getMealByDate(date: Date): Promise<Meal> {
        ...
        rows.forEach((row: IMealInfoRow) => {
             ...
             */
            const menu: string = row.DDISH_NM.replace(/([0-9]+\.)+/g, "")
                .replace(/\(\)/g, "")
                .replace(/\s/g, "")
                .replace(/\./g, "")
                .split("<br/>")
